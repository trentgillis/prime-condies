  ...
  return (
    <div className="flex w-full gap-2">
      <div className="flex flex-1 flex-col gap-1 rounded border border-zinc-700 bg-zinc-900 p-1.5 lg:p-2">
        ...
        <div className="text-sm font-semibold text-zinc-50">{area.weatherData.daily[0].rain ?? 0}&#8243;</div>
      </div>
      <div className="flex flex-1 flex-col gap-1 rounded border border-zinc-700 bg-zinc-900 p-1.5 lg:p-2">
        <div className="flex items-center gap-1">
        ...
        <div className="text-sm font-semibold text-zinc-50">{area.weatherData.current.humidity}%</div>
      </div>
      <div className="flex flex-1 flex-col gap-1 rounded border border-zinc-700 bg-zinc-900 p-1.5 lg:p-2">
        <div className="flex items-center gap-1">
        ...
        <div className="text-sm font-semibold text-zinc-50">{Math.round(area.weatherData.current.wind_speed)}mph</div>
      </div>
      <div className="hidden flex-1 flex-col gap-1 rounded border border-zinc-700 bg-zinc-900 p-1.5 lg:flex lg:p-2">
        ...
        <div className="text-sm font-semibold text-zinc-50">
          {new Date(area.weatherData.current.sunset * 1000).toLocaleString('en-US', {
            timeZone: area.weatherData.timezone,
            hour: 'numeric',
            minute: 'numeric',
          })}
        </div>
